...
import { 
  ChevronRight, CircleCheckBig, Hash, 
  PenTool, TriangleAlert, Users2 
} from "lucide-react";
...
const EventDashboardData = () => {
  ...
  // HARDCODE (Temp data)
  const currentTarget: number = 1257;
  const finalTarget: number = 5000;
  const sudahBayarMas: number = 410;
  let progressBar: number = (currentTarget / finalTarget) * 100;
  
  useEffect(() => {
    const timer = setTimeout(() => setProgress(progressBar), 2000)
    ...
    <>
      <div className="space-y-1.5">
        <div className="flex gap-1">
          ...
          <p className="text-sm font-bold">{`${currentTarget}/${finalTarget}`}</p> {/*hardcode*/}
        </div>
